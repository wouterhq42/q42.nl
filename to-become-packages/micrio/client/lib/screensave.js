...
Screensaver.prototype = {
	goto: function(index) {
		...
		(current = this.instances[index]
			|| (this.instances[index] = new Micrio({
				...
				current.camera.easeTo(.4+Math.random()*.2,.4+Math.random()*.2,1,that.speed,undefined,isFirst && Beziers.easeIn);
	...
	checkScroll: function(){
		...
		var offsetY = this.instances[this.currentIndex].camera.getRenderedOffset(this.container).top - (window.scrollY || window.pageYOffset);
